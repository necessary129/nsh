...
sDLL *screateDLL() {
	sDLL *newDLL = checkAlloc(calloc(1, sizeof *newDLL));
	return newDLL;
}
...
sDData sdGetData(DElement *element) { return element->data; }

DElement *sdNext(DElement *element) { return element->next; }

DElement *sdPrev(DElement *element) { return element->prev; }
